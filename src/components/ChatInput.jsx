  ...
  return (
    <div className="p-4 sm:p-6 bg-slate-900 border-t border-slate-800 shrink-0">
      ...
      <div className="max-w-5xl mx-auto flex gap-2 sm:gap-4 items-end bg-slate-800 p-2 sm:p-3 rounded-2xl border border-slate-700 focus-within:border-blue-500/50 focus-within:ring-4 transition-all">
        ...
        <button onClick={() => fileInputRef.current.click()} disabled={isTyping} className="p-3 rounded-xl text-slate-400 hover:text-slate-200 hover:bg-slate-700 disabled:opacity-50">
           📎
        </button>

        <input 
          type="text" className="flex-1 bg-transparent border-none text-slate-200 px-2 py-3 focus:outline-none placeholder-slate-500"
          placeholder="Type your health queries..." value={inputValue} onChange={(e) => setInputValue(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSendMessage()} disabled={isTyping}
        />
        
        <button onClick={handleVoiceInput} disabled={isTyping} className={`p-3 rounded-xl flex items-center justify-center ${isListening ? 'bg-red-500/20 text-red-500 animate-pulse' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-700'}`}>
          🎤
        </button>

        <button onClick={handleSendMessage} disabled={isTyping || (!inputValue.trim() && !selectedImage)} className="p-4 rounded-xl bg-blue-600 text-white font-semibold hover:bg-blue-500 disabled:opacity-50 shadow-md">
          ➤
        </button>
      </div>
